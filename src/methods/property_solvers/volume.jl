...
Main routine to calculate a volume, given a pressure, temperature, composition and intitial volume guess. each step is taken by locally approximating the EoS as an isothermal compressibility process.
...
!!! warning "Stability checks"
    ...
    ```
"""
function volume(model::EoSModel, p, T, z=SA[1.0]; phase=:unknown, threaded=true, vol0=nothing, extrapolate_density=false)
    return volume_impl(model, p, T, z, phase, threaded, vol0, extrapolate_density)
end
...
function volume_impl(model::EoSModel, p, T, z=SA[1.0], phase=:unknown, threaded=true, vol0=nothing, extrapolate_density=false)
    #Threaded version
